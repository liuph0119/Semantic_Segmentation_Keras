...
def residual_block(inputs, base_depth, depth, kernel_size, stride=1, rate=1, block_name="block1", unit_name="unit1", weight_decay=4e-5):
    ...
    x = Conv2D(base_depth, (kernel_size, kernel_size), strides=(stride, stride), dilation_rate= rate, padding="same", name=conv_name_base + "2b", kernel_regularizer=l2(weight_decay))(x)
...
def resnet_v2_50(input_shape, kernel_size=3, include_root=True):
...
def resnet_v2_101(input_shape, kernel_size=3, include_root=True):
...
def resnet_v2_152(input_shape, kernel_size=3, include_root=True):
...
def resnet_v2_200(input_shape, kernel_size=3, include_root=True):
